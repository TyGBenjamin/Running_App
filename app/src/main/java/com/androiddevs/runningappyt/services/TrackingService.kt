...
/**
 * [LifecycleService] used to track location while running.
 *
 * @constructor Create instance of [TrackingService]
 */
@AndroidEntryPoint
class TrackingService : LifecycleService() {
    ...
    private var _curNotificationBuilder: NotificationCompat.Builder? = null
    private val curNotificationBuilder: NotificationCompat.Builder get() = _curNotificationBuilder!!
    ...
    override fun onCreate() {
        super.onCreate()
        _curNotificationBuilder = baseNotificationBuilder
        postInitialValues()
    ...
    private fun startTimer() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            while (isTracking.value!!) {
                ...
                if (timeRunInMillis.value!! >= lastSecondTimestamp + Constants.Time.MILLI_SEC) {
                    timeRunInSeconds.postValue(timeRunInSeconds.value!! + 1)
                    lastSecondTimestamp += Constants.Time.MILLI_SEC
                }
    ...
    private fun updateNotificationTrackingState(isTracking: Boolean) {
        ...
        val flag = FLAG_IMMUTABLE or FLAG_UPDATE_CURRENT
    ...
    private fun startForegroundService() {
        ...
        timeRunInSeconds.observe(this) {
            if (!serviceKilled) {
                val formattedTime =
                    TrackingUtility.getFormattedStopWatchTime(it * Constants.Time.MILLI_SEC)
